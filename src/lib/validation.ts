...
export const phoneSchema = z.string().trim().regex(/^[+()\d\s.-]{7,25}$/, 'Enter a valid phone number.');
...
export const serviceSchema = z.object({ id: z.string(), name: nameSchema, description: z.string().trim().min(10, 'Add a description of at least 10 characters.').max(500), duration: z.number().int().min(15).max(240).multipleOf(15, 'Duration must be in 15-minute increments.'), price: z.number().min(0).max(10000), active: z.boolean(), color: z.string() });
