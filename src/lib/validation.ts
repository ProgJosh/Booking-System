...
export const bookingSchema = z.object({ customerId: z.string().min(1), serviceId: z.string().min(1), staffId: z.string().min(1), date: dateSchema, time: timeSchema, paymentMethod: z.enum(PAYMENT_METHODS, 'Choose a valid payment method.'), notes: z.string().max(1000) });
